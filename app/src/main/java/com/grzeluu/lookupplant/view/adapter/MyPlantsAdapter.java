...
import static com.grzeluu.lookupplant.utils.AlertDialogUtils.showDeletePlantDialog;
import static com.grzeluu.lookupplant.utils.ProgressUtils.daysFromLastAction;
import static com.grzeluu.lookupplant.utils.TimeUtils.DATE_FORMAT;
...
import com.grzeluu.lookupplant.databinding.ItemMyPlantBinding;
import com.grzeluu.lookupplant.model.UserPlant;
import com.grzeluu.lookupplant.view.AdviceBottomSheetDialogFragment;
import com.grzeluu.lookupplant.view.EditPlantFragment;
...
public class MyPlantsAdapter extends RecyclerView.Adapter<MyPlantsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyPlantsAdapter.ViewHolder holder, int position) {
        ...
        holder.binding.btnAdvices.setOnClickListener(v -> {
                        ...
                        @Override public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if (snapshot.exists()) {
                                for (DataSnapshot ds : snapshot.getChildren()) {
                                    String publicId = ds.getKey();
                                    AdviceBottomSheetDialogFragment sheet =
                                            AdviceBottomSheetDialogFragment.newInstance(publicId, name);
                                    sheet.show(((androidx.appcompat.app.AppCompatActivity) v.getContext()).getSupportFragmentManager(), "advices_sheet");
                                    return;
                                }
                            } else {
                                Toast.makeText(v.getContext(), "No public advices found", Toast.LENGTH_SHORT).show();
                            }
                        }
        ...
        holder.binding.btnEdit.setOnClickListener(v -> openEditFragment(plant));

    }

    private void openEditFragment(UserPlant plant) {
        Bundle bundle = new Bundle();
        bundle.putSerializable("USER_PLANT_EDIT", plant); // Для редактирования UserPlant
    ...
    private void initProgressBar(TextView text, ProgressBar progressBar, long frequency, Date lastAction) {
        ...
        if (daysLeft > 1) {
            text.setText(context.getString(R.string.days, daysLeft));
        } else if (daysLeft == 1) {
            text.setText(context.getString(R.string.one_day));
        } else {
            text.setText(context.getString(R.string.today));
        }
    }
